...
class TestCaseBaseConfiguration(ABC, BaseModel):
    ...
    port_role_handler: Optional[PortRoleHandler] = None
    duration: int
...
class IPv6Prefix(int):
    def to_ipv6(self) -> IPv6Address:
        return IPv6Address(int(self * "1" + (64 - self) * "0", 2))
    
    @classmethod
    def __get_pydantic_core_schema__(cls, source_type, handler):
        return core_schema.no_info_after_validator_function(
            cls, 
            core_schema.int_schema(ge=0, le=64)
        )
...
class IPV6AddressProperties(BaseModel, arbitrary_types_allowed=True):
    address: IPv6Address
    routing_prefix: IPv6Prefix  = IPv6Prefix(64)
    public_address: IPv6Address
    public_routing_prefix: IPv6Prefix = IPv6Prefix(64)
    gateway: IPv6Address
    ...
    def set_prefix(cls, v) -> IPv6Prefix:
        return IPv6Prefix(v)
...
class IPV4AddressProperties(BaseModel, arbitrary_types_allowed=True):
    address: IPv4Address
    routing_prefix: IPv4Prefix = IPv4Prefix(24)
    public_address: IPv4Address
    public_routing_prefix: IPv4Prefix = IPv4Prefix(24)
    gateway: IPv4Address
