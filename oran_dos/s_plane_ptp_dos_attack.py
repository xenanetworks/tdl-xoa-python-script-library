...
async def s_plane_ptp_dos(chassis_ip: str, port_str: str, username: str):

    _mid = int(port_str.split("/")[0])
    _pid = int(port_str.split("/")[1])

    print(f"#####################################################################")
    print(f"Test:               S-Plane PTP DoS Attack")
    ...
    async with testers.L23Tester(host=CHASSIS_IP, username=USERNAME, password="xena", port=22606, enable_logging=False) as tester:
        ...
        # await cli.port_config_from_file(port, "ptp_dos.xpc")
...
async def main():
    ...
    try:
        await s_plane_ptp_dos(CHASSIS_IP, PORT, USERNAME)
