...
# 2. Reserve a port. Must be Thor or Loki
# 3. Reset the port
...
async def prbs_ber_stats(chassis: str, username: str, port_str1: str, port_str2: str, duration: int):
    ...
    async with testers.L23Tester(host=chassis, username=username, password="xena", port=22606, enable_logging=False) as tester:
        ...
        if not isinstance(module_obj1, modules.Z400ThorModule):
            logging.info(f"Not Freya or Thor or Loki module")
            return None
        if not isinstance(module_obj2, modules.Z400ThorModule):
            logging.info(f"Not Freya or Thor or Loki module")
            return None
        ...
        for i in range(_serdes_count1):
            await port_obj1.serdes[i].prbs.control.set(prbs_seed=0, prbs_on_off=enums.PRBSOnOff.PRBSON, error_on_off=enums.ErrorOnOff.ERRORSOFF)
        ...
        _list = []
        for i in range(_serdes_count1):
            _list.append(port_obj1.serdes[i].prbs.control.set(prbs_seed=0, prbs_on_off=enums.PRBSOnOff.PRBSOFF, error_on_off=enums.ErrorOnOff.ERRORSOFF))
        await utils.apply(*_list)
