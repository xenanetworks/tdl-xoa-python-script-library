...
# live_plot
#---------------------------
async def live_plots(chassis: str, username: str, tx_port_str: str, rx_port_str: str, filter_idx: int, figure_title: str, duration: int, win_size: int, plot_interval: float):
    ...
    async with testers.L23Tester(host=chassis, username=username, password="xena", port=22606, enable_logging=False) as tester:
        ...
        axs[2].set(xlabel=f"Time ({plot_interval}s)", ylabel=f"ECN Count (port {rx_port_str})")
        ...
        for i in range(duration):
            x = i
            resp0, resp1, resp2 = await asyncio.gather(
                ...
                rx_port_obj.statistics.rx.obtain_filter_statistics(filter=filter_idx).get(), # Read the filtered traffic stats - packet count since cleared
            )
